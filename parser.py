...
class PartialParse(object):
    ...
    def get_oracle(self, graph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        second_dep = list(set(get_deps(graph.nodes[idx_second])) - set(idx_removed))

        if graph.nodes[idx_first]['head'] == idx_second and not first_dep:
            transition_id = self.right_arc_id
            deprel = graph.nodes[idx_first]['rel']
        elif graph.nodes[idx_second]['head'] == idx_first and not second_dep:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        minibatch = [partial_parsers[idx] for idx in idx_minibatch]
